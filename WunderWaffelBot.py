...
async def send_random_mixed(chat_id, amount=3, caption_base=""):
    try:
        ...
        response = requests.get("https://api.waifu.im/search", params=params, timeout=10)
...
async def send_waifu_by_tag(chat_id, tag, amount=3, caption_base=""):
    try:
        params = {"included_tags": [tag], "limit": amount, "is_nsfw": "true"}  # Всегда NSFW для этих тегов 🔥
        params.update(build_excluded_param())
...
async def cmd_nsfw(message: types.Message):
    await send_random_mixed(message.chat.id, amount=3, caption_base="Горяченькая случайная: ", is_nsfw=True)  # Только NSFW для /nsfw
